...
var RelayNetworkLayer = function () {
  ...
  RelayNetworkLayer.prototype.sendSubscription = function sendSubscription(subscriptionRequest) {
    ...
    require('fbjs/lib/invariant')(typeof implementation.sendSubscription === 'function', 'RelayNetworkLayer: does not support subscriptions.  Expected `sendSubscription` to be ' + 'a function.');

    var result = implementation.sendSubscription(subscriptionRequest);

    require('fbjs/lib/invariant')(result && typeof result.dispose === 'function', 'RelayNetworkLayer: `sendSubscription` should return an object with a ' + '`dispose` property that is a no-argument function.  This function is ' + 'called when the client unsubscribes from the subscription ' + 'and any network layer resources can be cleaned up.');
